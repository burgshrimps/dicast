...
class AlignmentAnnotatorIllumina:
    ...
    def __init__(self, bam_filename: str, chrom: str, sv_type: str, sample: str):
        ...
        self.max_fetch_reads = 10000  # Hard cap on reads fetched per bin; over-cap regions (centromeres, repeats) are zero-filled.
    ...
    def calculate_coverage_region(self, chrom: str, start: int, stop: int, suffix: str) -> pd.Series:
        ...
        # Retrieve all reads overlapping the region, clip their start/end positions to the
        # window, and use per-position read start/end deltas + cumsum to recover coverage.
        n = stop - start + 1
        rs_list: list = []
        re_list: list = []
    ...
    def calculate_coverage(self, df: pd.DataFrame, chrom_col: str, pos_col_left: str, pos_col_right: str, offset_left: int, offset_right: int, suffix: str, check_len: bool = False) -> pd.DataFrame:
        ...
        while i < len(df):
            ...
            if check_len:
                ...
                # Cache keyed on (chrom, left, right) — cheap hit for variants that share breakpoints.
                _cache_key = f'{df.loc[i, chrom_col]}_{left_border}_{right_border}'
                if _cache_key in coverage_dict:
                    df.loc[i, ['ill_cov_mean_' + suffix, 'ill_cov_std_' + suffix]] = coverage_dict[_cache_key]
                else:
                    df.loc[i, ['ill_cov_mean_' + suffix, 'ill_cov_std_' + suffix]] = self.calculate_coverage_region(df.loc[i, chrom_col], left_border, right_border, suffix)
                    coverage_dict[_cache_key] = df.loc[i, ['ill_cov_mean_' + suffix, 'ill_cov_std_' + suffix]]
    ...
    def calculate_read_based_features(self, chrom: str, start: int, end: int, sv_start: int, sv_end: int, suffix: str) -> pd.Series:
        ...
        for read in self.bam.fetch(chrom, start - 20, end + 20):
            ...
            reads_fetched += 1
            # Regions exceeding max_fetch_reads (e.g. centromeres / repeats) are not scorable;
            # zero all per-bin counters and exit the loop early.
